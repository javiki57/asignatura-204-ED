...
mapPQ f q = foldrPQ (\x s -> enqueue (f x) s) empty q

--mapPQ f c@(Node x q) = listToPQ (map f lista)  --encolamos en cada llamada recursiva un nodo aplicando la funcion.
--   where
--        lista = toList c
...
filterPQ p (Node x q)
        | p x       = enqueue x (filterPQ p q)
        | otherwise = filterPQ p q
...
foldrPQ f z Empty = z
foldrPQ f z (Node x restoCola) = f x (foldrPQ f z restoCola)
