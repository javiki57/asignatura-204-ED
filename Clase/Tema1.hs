...
máximoDeTres'' :: Int -> Int -> Int -> Int
máximoDeTres'' x y z = x `max` y `max` z -- max x (max y z)
...
códigosDe :: Char -> (Int, Int) -- código numérico es suficiente con Int
códigosDe x = (ord (toUpper x), ord (toLower x))     -- Con ord obtengo el valor del caracter.
...
{-
   ...
   expresiones:
     ...
     ( 1 < 2, if 5 < 7 then 'a' else 'b', -1, (7, 'c')) :: (Bool,Char,Double,(Int,Char))
