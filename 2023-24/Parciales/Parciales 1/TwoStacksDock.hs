...
isFirst :: Dock a -> Bool
isFirst s@(D s1 s2) = S.isEmpty s1 
...
isLast :: Dock a -> Bool
isLast s@(D s1 s2) = S.isEmpty (S.pop s2)
...
left s@(D s1 s2)
    | isFirst s = s
    | otherwise = D (S.pop s1) (S.push (S.top s1) s2)
...
right s@(D s1 s2 )
        | isLast s  = s
        | otherwise = D (S.push (S.top s2) s1) (S.pop s2)
...
delete :: Dock a ->  Dock a
delete s@(D s1 s2)
        | isEmpty s = error "Está vacía quillo"
        | isLast s  = if S.isEmpty s1 then empty else D (S.pop s1) (S.push (S.top s1) (S.pop s2))
        | otherwise = D s1 (S.pop s2)
...
listToDock :: [a] -> Dock a
listToDock d = foldr (insertl) empty d
