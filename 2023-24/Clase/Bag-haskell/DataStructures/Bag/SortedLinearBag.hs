...
bolsa1 :: Bag Char
bolsa1 = Node 'a' 2 (Node 'b' 3 (Node 'c' 2 (Node 'e'  1 Empty)))
...
delete x (Node y oy s)
      | x == y    = if oy > 1 then (Node y (oy-1) s) else s
...
occurrences_insert_1 x s = occurrences x (insert x s) == 2
occurrences_insert_2 x y s = x /= y ==> occurrences x (insert y s) == 1

-- transformadores
delete_empty x = delete x empty == Empty
delete_insert_1 x s = delete x (insert x s) == s
delete_insert_2 x y s  = x /= y ==> delete x (insert y s) == (insert y s)
...
union b1 Empty = b1
union Empty b2 = b2
union (Node x ox n) (Node y oy s)
      | x == y    = Node x (oy + ox) (union n s)
      | x < y     = Node x ox (union n (Node y oy s))
      | otherwise = Node y oy (union s (Node x ox n))
...
intersection (Node x ox n) (Node y oy s)
         | x == y    = Node x (min ox oy) (intersection n s)
         | x < y     = intersection n (Node y oy s) 
         | otherwise = intersection s (Node x ox n)
...
difference Empty b2 = b2
difference (Node x ox n) (Node y oy s)
         | x == y    = if (ox > oy) then (Node x (ox - oy) (difference n s)) else difference n s
         | x < y     = Node x ox (difference n (Node y oy s))
         | otherwise = difference (Node x ox n) s
